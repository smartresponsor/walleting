...
class ProviderEvent
{
    ...
    public function __construct(string $provider, string $externalId, string $eventType, array $payload, ?Uuid $id = null)
    {
        ...
        $this->id = $id ?? Uuid::v7(); $this->provider = $provider; $this->externalId = $externalId; $this->eventType = $eventType; $this->payload = $payload; $this->payloadHash = hash('sha256', json_encode($this->normalize($payload), JSON_THROW_ON_ERROR)); $this->status = ProviderEventStatus::Received; $this->receivedAt = new \DateTimeImmutable();
    }

    public function processFunding(Funding $funding): void { $this->assertReceived(); $this->funding = $funding; $this->complete(); }
    public function processWithdrawal(Withdrawal $withdrawal): void { $this->assertReceived(); $this->withdrawal = $withdrawal; $this->complete(); }
    public function markProcessed(): void { $this->assertReceived(); $this->complete(); }
    ...
    private function assertReceived(): void { if (ProviderEventStatus::Received !== $this->status) { throw new \LogicException('Only received events can be processed.'); } }
    private function complete(): void { $this->status = ProviderEventStatus::Processed; $this->processedAt = new \DateTimeImmutable(); }
    private function normalize(array $value): array { ksort($value); foreach ($value as &$item) { if (is_array($item)) { $item = $this->normalize($item); } } unset($item); return $value; }
    ...
    public function withdrawal(): ?Withdrawal { return $this->withdrawal; }
}

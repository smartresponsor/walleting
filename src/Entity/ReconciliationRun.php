...
class ReconciliationRun
{
    ...
    public function __construct(string $provider, string $runKey, ?Uuid $id = null)
    {
        ...
        $runKey = trim($runKey);
        if ('' === $provider || '' === $runKey) {
            throw new \InvalidArgumentException('Provider and run key are required.');
        }
        $this->id = $id ?? Uuid::v7();
    ...
    public function recordMismatch(): void
    {
        $this->assertRunning();
        ++$this->checkedCount;
        ++$this->mismatchCount;
    ...
    {
        if (!in_array($this->status, [ReconciliationRunStatus::Pending, ReconciliationRunStatus::Running], true)) {
            throw new \LogicException('Reconciliation cannot fail from its current status.');
    ...
    {
        if (ReconciliationRunStatus::Running !== $this->status) {
            throw new \LogicException('Reconciliation counters can only change while running.');
        }
    }

    public function status(): ReconciliationRunStatus
    {
        return $this->status;
    }
